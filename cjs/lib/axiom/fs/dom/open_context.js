...
"use strict";
var axiom$core$error$$ = require("axiom/core/error"), axiom$fs$base$open_context$$ = require("axiom/fs/base/open_context"), axiom$fs$dom$domfs_util$$ = require("axiom/fs/dom/domfs_util"), axiom$fs$path$$ = require("axiom/fs/path");
...
var DomOpenContext = function(domfs, path, arg) {
  ...
  this.onFileError_ = axiom$fs$dom$domfs_util$$.default.rejectFileError.bind(null, path.spec);
...
exports["default"] = DomOpenContext;
...
DomOpenContext.prototype.seek_ = function(arg) {
  ...
  if (start > fileSize) {
    ...
    return Promise.reject(new axiom$core$error$$.default.Runtime('reached end of file.'));
  }
    ...
    var onFileFound = function(entry) {
      ...
      if (mode.write && mode.truncate) {
        this.entry_.createWriter(
            function(writer) {
              writer.truncate(0);
              axiom$fs$dom$domfs_util$$.default.statEntry(entry).then(onStat).catch(onFileError);
            },
            reject);
        return;
      }

      axiom$fs$dom$domfs_util$$.default.statEntry(entry).then(function(value) {
        onStat(value);
      }).catch(function(e) {
        reject(e);
      });
...
DomOpenContext.prototype.write_ = function(arg) {
  if (!arg) {
    return Promise.reject(new axiom$core$error$$.default.Missing('arg'));
  }
  var dataType = arg.dataType || 'utf8-string';
  return new Promise(function(resolve, reject) {
...
exports.DomOpenContext = DomOpenContext;
