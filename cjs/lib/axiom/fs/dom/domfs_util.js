  ...
  return new Promise(function(resolve, reject) {
     var onMetadata = function(entry, metadata) {
       if (entry.isFile) {
         resolve({
           dataType: 'blob',
           mode: axiom$fs$path$$.default.Mode.R | axiom$fs$path$$.default.Mode.W | axiom$fs$path$$.default.Mode.K,
           mtime: new Date(metadata.modificationTime).getTime(),
           size: metadata.size
         });
       } else {
         resolve({
           mode: axiom$fs$path$$.default.Mode.R | axiom$fs$path$$.default.Mode.D,
           mtime: new Date(metadata.modificationTime).getTime(),
         });
       }
     };
     ...
     if ('getMetadata' in entry) {
       ...
       reject(new axiom$core$error$$.default.Runtime('entry has no getMetadata'));
     }
   });
 };
