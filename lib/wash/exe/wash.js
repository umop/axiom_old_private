...
import CmdLine from 'wash/cmd_line';
...
export var Wash = function(executeContext) {
  ...
  this.commandLine = new CmdLine(this.executeContext);
...
// NOTE: This processing is naive. It won't correctly account for
// double/single-quotes, multiple whitespace-separated arguments, etc.
// TODO(ussuri): Remove when all command lines go through CommandLine.parse.
Wash.prototype.interpolateUnescape_ = function(argv) {
  return string.unescape(this.executeContext.interpolateEnvs(argv));
};
...
Wash.prototype.parseArgv = function(argSigil, argv) {
  ...
  if (/[\{\[]/.test(argv.substr(0, 1))) {
    ...
    try {
      return JSON.parse(this.interpolateUnescape_(argv));
    } catch (ex) {
      throw new AxiomError.Runtime('Error parsing arguments: ' + ex);
    }

  } else {
    // TODO(ussuri): All these cases should go through commandLine.parse(),
    // as it accounts for single/double-quotes, subcommands, escaped chars, etc.
    if (argSigil == '$' || argSigil == '*')
      return this.interpolateUnescape_(argv);

    if (argSigil == '@')
      return this.interpolateUnescape_(argv).split(/\s+/g);

    // commandLine.parse() interpolates itself.
    if (argSigil == '%')
      return this.commandLine.parse(argv);
  }
};
